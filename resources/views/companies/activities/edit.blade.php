            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="overflow-hidden overflow-x-auto border-b border-gray-200 bg-white p-6">
                    <form action="{{ route('companies.activities.update', [$company, $activity]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                            <x-input-label for="rep_id" value="Representatives" />
                            <select name="rep_id" id="rep_id" class="mt-1 block w-full rounded-md border-gray-300
                            shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option>-- SELECT REPRESENTATIVE --</option>
                                @foreach($reps as $id => $name)
                                    <option value="{{ $id }}" @selected(old('rep_id', $activity->rep_id) === $id)>{{ $name }}</option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->get('rep_id')" class="mt-2" />
                        </div>
